...
class BNN(nn.Module):
  ...
  def forward(self, x):
    ...
    output = F.relu(self.fc7(output))
    output = F.relu(self.fc8(output))
    output = F.relu(self.fc9(output))
    output = F.relu(self.fc10(output))
  ...
  def model(self, x_data, y_data = None):
    ...
    fc7w_prior = Normal(loc=torch.zeros_like(self.fc7.weight), scale=torch.ones_like(self.fc7.weight)).to_event(2)
    fc7b_prior = Normal(loc=torch.zeros_like(self.fc7.bias), scale=torch.ones_like(self.fc7.bias)).to_event(1)

    fc8w_prior = Normal(loc=torch.zeros_like(self.fc8.weight), scale=torch.ones_like(self.fc8.weight)).to_event(2)
    fc8b_prior = Normal(loc=torch.zeros_like(self.fc8.bias), scale=torch.ones_like(self.fc8.bias)).to_event(1)

    fc9w_prior = Normal(loc=torch.zeros_like(self.fc9.weight), scale=torch.ones_like(self.fc9.weight)).to_event(2)
    fc9b_prior = Normal(loc=torch.zeros_like(self.fc9.bias), scale=torch.ones_like(self.fc9.bias)).to_event(1)

    fc10w_prior = Normal(loc=torch.zeros_like(self.fc10.weight), scale=torch.ones_like(self.fc10.weight)).to_event(2)
    fc10b_prior = Normal(loc=torch.zeros_like(self.fc10.bias), scale=torch.ones_like(self.fc10.bias)).to_event(1)
    ...
    priors = {
        ...
        'fc6.bias': fc6b_prior,  
        'fc7.weight': fc7w_prior, 
        'fc7.bias': fc7b_prior,  
        'fc8.weight': fc8w_prior, 
        'fc8.bias': fc8b_prior,  
        'fc9.weight': fc9w_prior, 
        'fc9.bias': fc9b_prior,  
        'fc10.weight': fc10w_prior, 
        'fc10.bias': fc10b_prior,
  ...
  def guide(self, x_data, y_data = None):
    ...
    # Seventh layer weight distribution priors
    fc7w_mu = torch.randn_like(self.fc7.weight)
    fc7w_sigma = torch.randn_like(self.fc7.weight)
    fc7w_mu_param = pyro.param("fc7w_mu", fc7w_mu)
    fc7w_sigma_param = softplus(pyro.param("fc7w_sigma", fc7w_sigma))
    fc7w_prior = Normal(loc=fc7w_mu_param, scale=fc7w_sigma_param).to_event(2)

    # Seventh layer bias distribution priors
    fc7b_mu = torch.randn_like(self.fc7.bias)
    fc7b_sigma = torch.randn_like(self.fc7.bias)
    fc7b_mu_param = pyro.param("fc7b_mu", fc7b_mu)
    fc7b_sigma_param = softplus(pyro.param("fc7b_sigma", fc7b_sigma))
    fc7b_prior = Normal(loc=fc7b_mu_param, scale=fc7b_sigma_param).to_event(1)

    # Eighth layer weight distribution priors
    fc8w_mu = torch.randn_like(self.fc8.weight)
    fc8w_sigma = torch.randn_like(self.fc8.weight)
    fc8w_mu_param = pyro.param("fc8w_mu", fc8w_mu)
    fc8w_sigma_param = softplus(pyro.param("fc8w_sigma", fc8w_sigma))
    fc8w_prior = Normal(loc=fc8w_mu_param, scale=fc8w_sigma_param).to_event(2)

    # Eighth layer bias distribution priors
    fc8b_mu = torch.randn_like(self.fc8.bias)
    fc8b_sigma = torch.randn_like(self.fc8.bias)
    fc8b_mu_param = pyro.param("fc8b_mu", fc8b_mu)
    fc8b_sigma_param = softplus(pyro.param("fc8b_sigma", fc8b_sigma))
    fc8b_prior = Normal(loc=fc8b_mu_param, scale=fc8b_sigma_param).to_event(1)

    # Ninth layer weight distribution priors
    fc9w_mu = torch.randn_like(self.fc9.weight)
    fc9w_sigma = torch.randn_like(self.fc9.weight)
    fc9w_mu_param = pyro.param("fc9w_mu", fc5w_mu)
    fc9w_sigma_param = softplus(pyro.param("fc9w_sigma", fc9w_sigma))
    fc9w_prior = Normal(loc=fc9w_mu_param, scale=fc9w_sigma_param).to_event(2)

    # Ninth layer bias distribution priors
    fc9b_mu = torch.randn_like(self.fc9.bias)
    fc9b_sigma = torch.randn_like(self.fc9.bias)
    fc9b_mu_param = pyro.param("fc9b_mu", fc9b_mu)
    fc9b_sigma_param = softplus(pyro.param("fc9b_sigma", fc9b_sigma))
    fc9b_prior = Normal(loc=fc9b_mu_param, scale=fc9b_sigma_param).to_event(1)

    # Tenth layer weight distribution priors
    fc10w_mu = torch.randn_like(self.fc10.weight)
    fc10w_sigma = torch.randn_like(self.fc10.weight)
    fc10w_mu_param = pyro.param("fc10w_mu", fc10w_mu)
    fc10w_sigma_param = softplus(pyro.param("fc10w_sigma", fc10w_sigma))
    fc10w_prior = Normal(loc=fc10w_mu_param, scale=fc10w_sigma_param).to_event(2)

    # Tenth layer bias distribution priors
    fc10b_mu = torch.randn_like(self.fc10.bias)
    fc10b_sigma = torch.randn_like(self.fc10.bias)
    fc10b_mu_param = pyro.param("fc10b_mu", fc10b_mu)
    fc10b_sigma_param = softplus(pyro.param("fc10b_sigma", fc10b_sigma))
    fc10b_prior = Normal(loc=fc10b_mu_param, scale=fc10b_sigma_param).to_event(1)
